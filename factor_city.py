...
class FactorCityApp:
    ...
    def deploy_custom_tower(self) -> None:
        ...
        if not inserted:
            # Grid is completely full — no empty slot found. Overwriting is
            # the only option, but do it visibly rather than silently
            # destroying whatever building was at (0,0) without any
            # indication to the user that a replacement happened.
            old_val = self.grid[0][0]
            self.grid[0][0] = val
            self.inspector_label.text = (
                f"Grid full — replaced building at (0,0)\n"
                f"(was {old_val}, now {val})"
            )
